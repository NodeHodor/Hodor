...
//         this.test("simple equality", function() {
            var parsedQuery = parse({})
            parsedQuery.matches({a:'a', b:'b'})
            parsedQuery.matches({a:'b', b:'a'})
            parsedQuery.matches({})

            var parsedQuery = parse({a:null})
            parsedQuery.matches({a:null})
            parsedQuery.matches({})
            !parsedQuery.matches({a:'b'})
...
//         this.test("simple operator queries", function() { //{b: {$gt: '3'}}
            ...
            parsedQuery.matches({a:4})
            !parsedQuery.matches({a:3})
            !parsedQuery.matches({})

            var parsedQuery = parse({a: {$lt: 3}})
            parsedQuery.matches({a:2})
            !parsedQuery.matches({a:3})
            !parsedQuery.matches({})
            ...
            parsedQuery.matches({a:[1,2,2,2,1]})
            !parsedQuery.matches({a:2})
            !parsedQuery.matches({a:[1,2,3]})
            !parsedQuery.matches({})

            var parsedQuery = parse({a: {$mod: [4,3]}})
            parsedQuery.matches({a:3})
            parsedQuery.matches({a:7})
            !parsedQuery.matches({a:5})
            !parsedQuery.matches({})

            var parsedQuery = parse({a: {$exists: true}})
            parsedQuery.matches({a:{x:1}}) // object value
            parsedQuery.matches({a:false}) // primitive value
            !parsedQuery.matches({b:{b:'a'}, c:'a'})
            !parsedQuery.matches({})

            // more complex queries for $exists
            var parsedQuery = parse({'a.b': {$exists: true}})
            parsedQuery.matches({a:[{b:1},{x:'moose'}]}) // array element
            !parsedQuery.matches({})


            var parsedQuery = parse({a: {$regex: /x+/}})
            parsedQuery.matches({a:'x'})
            parsedQuery.matches({a:'xxx'})
            parsedQuery.matches({a:'xxxwaef'})
            !parsedQuery.matches({a:''})
            !parsedQuery.matches({})

            var parsedQuery = parse({a: {$size: 2}})
            parsedQuery.matches({a:[1,2]})
            !parsedQuery.matches({a:[1]})
            !parsedQuery.matches({a:2})
            !parsedQuery.matches({})

            var parsedQuery = parse({a: {$elemMatch: {x:5}}})
            parsedQuery.matches({a:[{x:5}, {y:4}]})
            !parsedQuery.matches({a:[{x:4}, {y:5}]})
            !parsedQuery.matches({})

            var parsedQuery = parse({a: {$elemMatch: {$gt:5}}})
            parsedQuery.matches({a:[4,5,6]})
            !parsedQuery.matches({a:[3,4,5]})

            var parsedQuery = parse({a: {$not: {$gt:5}}})
            parsedQuery.matches({a:4})
            parsedQuery.matches({})
            !parsedQuery.matches({a:6})
            ...
            parsedQuery.matches({a:3,b:6})
            !parsedQuery.matches({a:3,b:3})
            !parsedQuery.matches({a:6,b:3})
            !parsedQuery.matches({})
...
//         this.test("basic and", function() {
            ...
            parsedQuery.matches({a:5, b:6, c:7})
            !parsedQuery.matches({a:5, c:6})
//         })
...
//         this.test("dot notation", function() {
            ...
            !parsedQuery.matches({a:5})
            !parsedQuery.matches({a:{b:6}})
            !parsedQuery.matches({b:6})

            var parsedQuery = parse({'a.0': 5})
            parsedQuery.matches({a:[5,6,7]})
            !parsedQuery.matches({a:[6,6,7]})
            !parsedQuery.matches({a:[6,5,7]})
            !parsedQuery.matches({a:5})

            var parsedQuery = parse({'a.0.x': 5})
            parsedQuery.matches({a:[{x:5}]})
            !parsedQuery.matches({a:[6,6,7]})
            !parsedQuery.matches({a:[{x:6}, {x:5}]})

            // matching any element's subdocument
            var parsedQuery = parse({'a.x': 5})
            parsedQuery.matches({a:[0, {x:5}]})
            !parsedQuery.matches({a:[5,6,7]})
            !parsedQuery.matches({a:[{x:6}, {x:7}]})

            // more complex array subdocuments
            var parsedQuery = parse({'a.x.y.z': 5})
            parsedQuery.matches({a:[{x:[{y:[{z:5}]}]}]})
            !parsedQuery.matches({a:[{x:[{y:[{z:6}]}]}]})
//         })

//         this.test('array equality and element match', function() {
            var parsedQuery = parse({a: [1,2,3]})
            parsedQuery.matches({a:[1,2,3]})
            !parsedQuery.matches({a:[0,1,2,3]})
            !parsedQuery.matches({a:2})
            ...
            parsedQuery.matches({a:[5,6,7]})
            parsedQuery.matches({a:[6,7]})
            !parsedQuery.matches({a:[1,2,3]})
//         })

//         this.test("object equality", function() {
            var parsedQuery = parse({a: {x:1,y:2}})
            parsedQuery.matches({a:{x:1,y:2}})
            !parsedQuery.matches({a:{y:2,x:1}})
            !parsedQuery.matches({a:{x:1}})
            !parsedQuery.matches({x:1})
//         })
...
//     this.test('$in array match', function() {
        ...
        parsedQuery.matches({ tags:['a', 'b', 'c'] });
        parsedQuery.matches({ tags:['a'] });
        parsedQuery.matches({ tags:'a' });

        !parsedQuery.matches({ tags:[] });
        !parsedQuery.matches({ tags:['d'] });
        !parsedQuery.matches({ tags:['d', 'e'] });
        !parsedQuery.matches({ tags:null });
        !parsedQuery.matches({ tags:'' });
//     });
