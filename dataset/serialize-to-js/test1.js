...
var isLessV12 = parseInt(process.versions.node.split('.')[0]) < 12
var serialize = require('serialize-to-js').serialize;

// describe('serialize-to-js', function () {
  function test (name, inp, exp, unsafe) {
    // it(name, function () {
      var res = serialize(inp, { unsafe })
...
//     it('serializes function with unsafe chars', function () {
      function xss () {
        var str = '</script><script>alert(\'xss\')//'
        var o = { '\\": 0}; alert(\'xss\')//': 0, str }
        return o
      }
      var res = serialize(xss)
        .replace(/\n\s+/mg, '\n ')
      ...
      //   ' }'
      // )
//       var fn = new Function('return ' + res)()
...
//       var res = eval(serialize(new Date('Invalid'))) // eslint-disable-line no-eval
//       assert.strictEqual(res.toString(), new Date('Invalid').toString())
// //     })
// //     it('shall unmarshal Set', function () {
//       var set = new Set(['a', 1.2, true, ['b', 3], { c: 4 }])
//       var res = eval(serialize(set)) // eslint-disable-line no-eval
//       // assert.deepStrictEqual(Array.from(res), Array.from(set))
// //     })
// //     it('shall unmarshal Map', function () {
//       var map = new Map([['a', 'a'], [1.2, 1.2], [true, true], [['b', 3], ['b', 3]], [{ c: 4 }, { c: 4 }]])
//       var res = eval(serialize(map)) // eslint-disable-line no-eval
...
// //     it('Object ignore circularity', function () {
      var opts = { ignoreCircular: true }
      var o = {}
      o.self = o
      var res = serialize(o, opts)
      var exp = '{self: {/*[Circular]*/}}'
      assert.strictEqual(res, exp)
//     })
...
//       var arr = []
//       arr[0] = arr
//       var res = serialize(arr, { ignoreCircular: true })
//       var exp = '[[/*[Circular]*/]]'
//       assert.strictEqual(res, exp)
// //     })
...
// //     it('Set ignore circularity', function () {
      ...
      set.add(set)
      set.add('foo')
      var res = serialize(set, { ignoreCircular: true })
      var exp = 'new Set([undefined /*[Circular]*/, "foo"])'
      // assert.strictEqual(res, exp)
// //     })
...
// //     it('Map ignore circularity', function () {
      ...
      map.set('self', map)
      map.set('foo', 'bar')
      var res = serialize(map, { ignoreCircular: true })
      var exp = 'new Map([["self", undefined /*[Circular]*/], ["foo", "bar"]])'
      assert.strictEqual(res, exp)
// //     })
// //   })
// // })
