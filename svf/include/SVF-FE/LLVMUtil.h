...
{
    return ThreadAPI::getThreadAPI()->isTDFork_P2(cs);
}
inline bool isThreadForkCall(const Instruction *inst)
{
    return ThreadAPI::getThreadAPI()->isTDFork_P2(inst);
}
//@}
...
bool compareArgType(llvm::Type* first, llvm::Type* second);
bool compareStructType(StructType * firstStructType, StructType* secondStructType);
bool compareFunctionType(FunctionType* first, FunctionType* second);
bool isComplexFunctionType(FunctionType* funcType);
