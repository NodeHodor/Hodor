...
public:
    enum TD_TYPE
    {
        ...
        TD_FORK_P0,         /// create a new thread
        TD_FORK_P2,         /// create a new thread
...
public:
    ...
    //@{
    inline bool isTDFork_P2(const Instruction *inst) const
    {
        return getType(getCallee(inst)) == TD_FORK_P2;
    }
    inline bool isTDFork_P2(CallSite cs) const
    {
        return isTDFork_P2(cs.getInstruction());
    }
    //@}
    ...
    {
        return isHareParFor(cs.getInstruction());
    }
    //@}
    ...
    {
        assert(isTDFork_P2(inst) && "not a thread fork function!");
