...
void PointerAnalysis::printIndCSTargets()
{
    ...
    auto it = callEdges.begin();
    auto eit = callEdges.end();
    for (; it != eit; ++it)
    {
            ...
            ii != ie; ii++)
    {
        ...
        if(ObjVar* objPN = SVFUtil::dyn_cast<ObjVar>(pag->getGNode(*ii)))
        {
            ...
            if(obj->isFunction())
            {
                ...
                auto funcFunctionType = callee->getLLVMFun()->getFunctionType();
                auto csFunctionType = SVFUtil::getLLVMCallSite(cs->getCallSite()).getFunctionType();

                bool argMatch = compareFunctionType(funcFunctionType, csFunctionType);

                if (!argMatch) {
                    // 参数类型不匹配，过滤并且记录
                    GlobalContext::falseCallsiteFunctionsMap[cs->getCallSite()].insert(calleefun);
                    continue;
                }
                ...
                if(0 == getIndCallMap()[cs].count(callee))
                {
                    ...
                    // 正确的间接调用指向，记录
                    auto callbase = getLLVMCallSite(cs->getCallSite());
                    if (!llvm::isa<GlobalAlias>(callbase.getCalledValue()))
                        GlobalContext::trueCallsiteFunctionsMap[cs->getCallSite()].insert(calleefun);
...
{
    if(ThreadAPI::getThreadAPI()->isTDFork_P2(cs->getCallSite()))
        return true;
    else
