...
PTACallGraph* ThreadCallGraphBuilder::buildThreadCallGraph(SVFModule* svfModule)
{
    ...
    for (SVFModule::llvm_const_iterator fi = svfModule->llvmFunBegin(), efi = svfModule->llvmFunEnd(); fi != efi; ++fi)
    {
        ...
        for (const_inst_iterator II = inst_begin(*fun), E = inst_end(*fun); II != E; ++II)
        {
            ...
            // handle __clone
            else if (auto _callee = getCallee(inst)) {
                if (Options::syscallPass && _callee->getLLVMFun()->getName().equals("__clone")) {
                    auto callbase = getLLVMCallSite(inst);
                    auto firstArg = callbase.getArgument(0);
                    const CallICFGNode* callIcfgNode = icfg->getCallICFGNode(inst);
                    auto* caller = callIcfgNode->getCaller();
                    if (auto * forkee = llvm::dyn_cast<Function>(firstArg)) {
                        auto* callee = getDefFunForMultipleModule(forkee);
                        cg->addDirectCallGraphEdge(callIcfgNode, caller, callee);
                    } else if (auto * selectInst = llvm::dyn_cast<SelectInst>(firstArg)) {
                        auto true_func = selectInst->getTrueValue();
                        auto false_func = selectInst->getFalseValue();
                        if (llvm::isa<Function>(true_func) && llvm::isa<Function>(false_func)) {
                            auto* true_callee = getDefFunForMultipleModule(llvm::cast<Function>(true_func));
                            auto* false_callee = getDefFunForMultipleModule(llvm::cast<Function>(false_func));
                            cg->addDirectCallGraphEdge(callIcfgNode, caller, true_callee);
                            cg->addDirectCallGraphEdge(callIcfgNode, caller, false_callee);
                        }
                    } else {
