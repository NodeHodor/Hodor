...
// void LLVMModuleSet::initialiseDeadArgument() {
//
//     std::string func_name = "initialize_ftell";
//     std::vector<Function*> initcalls;
//
//     // std::vector<Type*> argtype(0);
//     // Type * i8ptr = PointerType::getInt8PtrTy(getContext());
//     // FunctionType *FT = FunctionType::get(i8ptr, argtype, false);
//     // Function *dummy_malloc = Function::Create(FT, Function::ExternalLinkage, "dummy_malloc");
//     // svfModule->addFunctionSet(dummy_malloc);
//     Function * fopen_func = nullptr;
//     Function * ftell_func = nullptr;
//
//     for (auto funcIter = svfModule->llvmFunBegin(); funcIter < svfModule->llvmFunEnd(); funcIter++) {
//         auto * func = *funcIter;
//         auto * retType = func->getReturnType();
//         while (retType->isPointerTy()) { retType = retType->getPointerElementType(); }
//
//         if (auto * structType = llvm::dyn_cast<StructType>(retType)) {
//             if (structType->getStructName().contains("struct._IO_FILE") && func->getName().equals("fopen") ) {
//                 fopen_func = func;
//                 break;
//             }
//         }
...
//     for (auto funcIter = svfModule->llvmFunBegin(); funcIter < svfModule->llvmFunEnd(); funcIter++) {
//         auto * func = *funcIter;
//         if (func->getName().equals("ftell"))
//             ftell_func = func;
//     }
//
//     std::vector<Type*> argtype(0);
//     FunctionType *FT = FunctionType::get(Type::getVoidTy(getContext()), argtype, false);
//     Function *F = Function::Create(FT, Function::ExternalLinkage, func_name);
//     BasicBlock *block = BasicBlock::Create(getContext(),"begin",F);
//     IRBuilder Builder(block);
//     std::vector<Value*> args;
//     for (auto &arg : ftell_func->args()){
//         if (arg.getType()->isPointerTy()){
//             auto call_inst = Builder.CreateCall(fopen_func);
//             auto cast_inst = Builder.CreateBitCast(call_inst, arg.getType());
//             args.push_back(cast_inst);
//         }
//         else{
//             auto alloc_inst = Builder.CreateAlloca(arg.getType(), 0, nullptr, "");
//             auto load_inst = Builder.CreateLoad(alloc_inst);
//             args.push_back(load_inst);
//         }
//         Builder.CreateCall(ftell_func,args);
//         Builder.CreateRetVoid();
//         initcalls.push_back(F);
//         llvm::outs() << *F;
//     }
//     for (auto  FF:initcalls){
//         svfModule->addFunctionSet(FF);
//     }
// }
