...
namespace SVF
{
    ...
    const llvm::cl::opt<bool> Options::syscallPass(
            "syscallPass",
            llvm::cl::init(false),
            llvm::cl::desc("extract syscall")
            );

    const llvm::cl::opt<bool> Options::libuvPass(
            "libuvPass",
            llvm::cl::init(false),
            llvm::cl::desc("handle libuv")
    );

    const llvm::cl::opt<std::string> Options::resultFile(
            "output",
            llvm::cl::init(""),
            llvm::cl::desc("the path of json result output")
    );
} // namespace SVF.
